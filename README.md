...
- **트랜잭션 로직 단순화 및 `ITxClient` 도입** — `factory.ts`의 `transaction()` 내부에서 `wrappedTx: IDbClient`를 완전히 재선언하던 중복 코드를 `makeClient()` 내부 헬퍼로 일원화. 외부 클라이언트와 트랜잭션 내부 클라이언트 모두 동일한 `makeClient(raw, parentTraceId?)` 호출로 생성하며, `parentTraceId` 유무만 다름. `lib/db/types.ts`에 `ITxClient = Omit<IDbClient, 'transaction'>` 타입을 추가하여 `transaction()` 콜백이 받는 `tx` 파라미터에서 중첩 트랜잭션 호출을 런타임 throw 대신 **컴파일 타임 타입 에러**로 차단. `IDbClient.transaction` 및 `IDbProvider.withTransaction` 콜백 시그니처도 `ITxClient`로 변경. Server Action에서 헬퍼 함수 파라미터를 `{ query: typeof db.query }` workaround 대신 `ITxClient`로 명확하게 표현 가능. `lib/db` barrel에서 `ITxClient` re-export 추가
- **트랜잭션 로그 상관관계(parentTraceId)** — `factory.ts`의 `transaction()`이 자체 `txTraceId`를 발급하고, 내부 `tx.query` / `tx.execute` 호출을 `withLifecycle`로 다시 감싸 개별 traceId와 함께 `parentTraceId = txTraceId`를 로그에 기록. 트랜잭션 블록 안에서 실행된 여러 쿼리를 하나의 트랜잭션으로 묶어 추적 가능. `QueryOptions.parentTraceId` 필드를 `lib/db/types.ts`에 추가
