...
- **트랜잭션 로그 상관관계(parentTraceId)** — `factory-new.ts`의 `transaction()`이 자체 `txTraceId`를 발급하고, 내부 `tx.query` / `tx.execute` 호출을 `withLifecycle`로 다시 감싸 개별 traceId와 함께 `parentTraceId = txTraceId`를 로그에 기록. 트랜잭션 블록 안에서 실행된 여러 쿼리를 하나의 트랜잭션으로 묶어 추적 가능. `QueryOptions.parentTraceId` 필드를 `lib/db/types.ts`에 추가
...
- **`factory.ts` `query()` 반환 타입 명시화** — 레지스트리 방식 `getDb`의 `query` 시그니처를 `Promise<T[]>` → `Promise<QueryResult<T>>`로 수정하여 `IDbClient` 인터페이스와 일치시킴 (환경변수 팩토리와 동일한 `{ columns, rows }` 구조 반환)
...
- **DB 풀 워밍업(`warmupDb`)** — `factory-new.ts`에 `warmupDb(name)` 함수 추가. 환경변수 resolver로 DSN/풀 옵션을 해석한 뒤 `provider.warmup()`을 호출하여 풀을 선제 생성. 이미 생성된 풀은 provider 캐시가 no-op 처리. `IDbProvider` 인터페이스에 `warmup(dbName, dsn, pool)` 시그니처를 추가하고 Oracle 프로바이더(`oracleWarmup`)에서 `getPool()` 호출로 구현
