...
### API & 미들웨어
- **Route Handler** — `/api/hello`, `/api/revalidate`, `/api/emp` (Oracle DB 연동)
- **Middleware** — 전역 요청 가로채기 및 헤더 조작
- **Private Folder** — `_components` (라우팅 제외)

### DB 연동
- **Oracle DB 통합** — scott.emp 테이블 조회 예제
  - `/emp` 페이지: MultiSelect 필터로 사원명 선택 및 조회
  - `/api/emp` Route Handler: WHERE IN 동적 쿼리 구성
...
```tsx
import { Button, Input, Select, Panel, Badge, Modal, MultiSelect } from '@/components/control'
```
...
│   ├── hello/route.ts
│   ├── revalidate/route.ts
│   └── emp/route.ts       # Oracle DB scott.emp 조회
│
...
8. **Database** — [EMP 조회](http://localhost:3000/emp) (Oracle DB 연동 Route Handler + MultiSelect 필터)
