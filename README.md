...
- **Virtualization + Cascading Select** — `/search` 페이지
  ...
  - `fetchItemDataSource` Server Action으로 조건 기반 데이터소스 조회
...
- **Input** — label, error, helperText, 다양한 type 지원 (`text`/`password`/`email`/`number`/`search`/`select`). `select` 타입은 `@tanstack/react-virtual` 기반 가상화 드롭다운 멀티셀렉트 제공
...
```tsx
import { Button, Input, Select, Panel, Badge, Modal, MultiSelect, Tab, TabSub, SearchPanel } from '@/components/control'
```
