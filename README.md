...
- **useActionState + useMemo 캐스케이딩 DataSource** — `/new-search` 페이지
  ...
  - **렌더링 에러 격리**: `ErrorBoundary` 컴포넌트로 Grid/Chart 영역을 각각 래핑 — 렌더 단계 에러 발생 시 fallback UI 표시, SearchPanel은 정상 동작 유지
...
- **CompGrid** — 가상화 데이터 그리드 (Compound Component 패턴). `loading` prop이 true이면 내장 스켈레톤 자동 렌더링. `<Suspense>` 래핑 불필요. `CompGrid.Skeleton` 단독 사용 가능. `GridColumn` 타입으로 컬럼 구성, `render` 함수로 셀 커스터마이징 지원. `onCellDoubleClick(value, column, row, rowIndex)` prop으로 셀 더블클릭 드릴다운 지원(핸들러 지정 시 셀에 `cursor-pointer` 자동 적용)
