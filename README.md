...
### appId/deptSite 라우팅·인증 DB 연동 전환 + 5자리 넘버링 (2026-06-03)
기존 "화면(appId)+지역(deptSite)" 체계(아래 별도 섹션)를 **하드코딩/Mock·모듈 TTL 캐시 기반에서 실제 DB(`SCOTT.TEST_USER` / `SCOTT.TEST_APP`) 조회 기반**으로 전환하고, appId 넘버링을 `A001`(영문1+숫자3) → `A00001`(영문1+숫자5)로 변경. proxy 진입 요청을 prefetch / navigation 으로 구분하여 **prefetch 시에는 권한·지원여부 DB 조회를 생략**한다.

- **proxy 인증을 `getToken` → `auth()` 세션 기반으로 교체** — JWT 디코드(`next-auth/jwt` `getToken`)와 하드코딩된 mock 플래그(`hasKrFlag`/`tempRole`/`tempAppIds`)를 제거하고 `await auth()` 세션으로 단일화. 비로그인 시 원래 경로를 `callbackUrl` 로 넘겨 `/login` 으로 redirect
- **prefetch / navigation 구분 도입** — `next-router-prefetch`(뷰포트 자동 prefetch) + `next-action`(Server Action) 헤더로 `isNavigation = !(isPrefetch || isServerAction)` 판정. prefetch 요청에서는 `getDeptSites()` DB 조회와 deptSite 권한 검사를 **건너뛰고**, 실제 화면 이동에서만 수행하여 뷰포트 prefetch가 매번 DB를 때리는 것을 차단. ⚠️ 클라이언트 Router Cache 히트(특히 back/forward, `prefetch={true}`) 시 proxy 자체가 실행되지 않을 수 있으므로, 인가의 **최종 경계는 Server Component / Server Action** 에 둘 것을 권장
- **`getDeptSites()` 실시간 DB 조회로 변경** — 하드코딩 `APP_REGISTRY` + 60초 모듈 TTL 캐시(`getDeptSitesCached`)를 제거. `SCOTT.TEST_APP` 의 `HAS_KR/JP/CN_FLAG` 를 `appId` 로 조회해 지원 deptSite 배열을 반환(미존재 시 `[]`). `Promise<DeptSites[]>` 비동기 시그니처로 통일
- **인증 쿼리를 `EMP` → `SCOTT.TEST_USER` 로 변경** — `authorize()` 가 `USER_ID`(username) / `USER_NAME` / `HAS_KR/JP/CN_FLAG` 를 조회. JWT/Session 콜백과 `types/next-auth.d.ts` 의 User/Session/JWT 타입을 `userId`/`userName`/`hasKrFlag`/`hasJpFlag`/`hasCnFlag` 로 교체(기존 `role`/`empno`/`name` 필드 및 방대한 SSO 가이드 주석 제거). deptSite 권한은 세션 플래그(`session.user.hasKrFlag === 'Y'` 등)로 검증
- **appId 넘버링 5자리 전환** — `app/A001`·`app/A002` 삭제, `app/A00001`·`app/A00002` 신규. matcher 정규식을 `/((?!_next/...).*)` → `'/([A-Za-z]\\d{5}(?:/.*)?)'` 로 좁혀 **appId 형식(영문1+숫자5) 경로에서만 proxy 실행**(불필요한 `auth()` 호출 제거). `lib/constants.ts` 네비게이션 항목과 `app/A00001` 의 prefetch 체험 Link 도 5자리로 갱신
- **deptSite 기본값 `'KR'` 통일** — deptSite 세그먼트가 없으면 redirect 대신 `'KR'` 로 간주(`proxy.ts`, `lib/hooks/useAppInfo.ts`의 `deptSite ?? 'KR'`). 기존 "플래그 우선순위로 default deptSite 결정 후 `/A001/KR` redirect" 로직 제거. rewrite 시 `search`(쿼리스트링) 보존
- **`appInfo` 주입 단순화** — `db-new2.ts` 의 `readAppInfo()` try/catch 헬퍼를 제거하고 `getDb().run()` 내부에서 `headers()` 를 직접 읽어 `appInfo` 를 항상 객체로 구성. `runOracle`/`runPg` 의 `appInfo` 파라미터 타입을 `AppInfo | undefined` → `AppInfo` 로 변경
- **로그인 페이지 단일 Client Component 화** — `Suspense` + `LoginForm` 분리 구조를 `app/login/page.tsx` 단일 `'use client'` 컴포넌트로 통합. `useSession` 으로 `authenticated` 가 되면 `callbackUrl`(없으면 `/A00001`)로 단일 이동하여 중복 네비게이션·플리커 방지
- **사이드 변경** — `Header.tsx` 사용자 표시를 `session.user.name` → `session.user.userId` 로, db provider 로그 prefix("테스트 테스트 테스트" → "getDb")를 정리

### DB 레이어 provider 파일 분리 + logger-new 재설계 (2026-05-25)
`db-new2.ts` 의 `runOracle` / `runPg` 가 한 파일에 같이 있으면서 `oracledb` / `pg` 양쪽 드라이버 import 가 강제로 묶이는 문제(번들 사이즈, 타입/옵션 충돌 위험)를 정리하여 provider 별 파일로 분리. 동시에 `lib/db/logger-new.ts` 가 더 이상 구버전 `db.ts` 의 `getPool` / `getProviderOption` 을 의존하지 않도록 `getDb()` 위에서 자기 자신을 다시 호출하는 구조로 재작성.

- **`lib/db/db-oracle.ts` 신규** — `OracleConfig` 타입, `ensureOracleThick()`(Thick 모드 + Instant Client `libDir` 로드, `globalThis.__myapp_oracle_thick_initialized__` SSOT), `getPoolStore()`(풀 캐시 `globalThis.__myapp_db_pool_cache__`), `runOracle<R>(name, config, isTransaction, callback, userInfo)` 본체를 한 파일에 집약. 풀 생성·`getConnection`·`execute`(`OUT_FORMAT_OBJECT` + `metaData.dbTypeName` → `'number'|'date'|'string'` 인라인 매핑)·commit/rollback·release 까지 단일 파일 안에서 위→아래 흐름으로 읽힘
- **`lib/db/db-pg.ts` 신규** — `PgConfig` 타입, `runPg<R>(...)` 본체. `pg.Pool` 생성·`pool.connect`·`pool.query`(컬럼 `dataTypeID(oid)` → `'bool'|'number'|'date'|'string'` 인라인 매핑)·`BEGIN/COMMIT/ROLLBACK`·`release` 를 oracle 과 같은 시그니처로 작성. 풀 캐시는 동일한 `globalThis` 키를 공유하여 oracle/pg 가 같은 `Map<string, oracledb.Pool | PgPool>` 안에서 name 충돌 없이 공존
- **`lib/db/db-new2.ts` 슬림화** — 진입 함수 `getDb({ name, isTransaction, isUserLess })` 만 남기고 provider 실행 로직 전부 제거. 본문은 `auth() → JSON.parse(env.DB_CONNECTION__<NAME>) → switch(config.providerName)` 두 case 만 분기하여 `runOracle` / `runPg` 위임. `DbConfig` 는 `OracleConfig | PgConfig` discriminated union 으로 재정의되어 각 case 안에서 자동 좁힘. `oracledb` / `pg` 의 직접 import 와 풀 캐시 / Thick init 헬퍼는 전부 빠짐
- **`lib/db/logger-new.ts` 재설계 — `getDb()` 재사용 + ALS 가드** — 구버전 `db.ts` 의 `getPool('MAIN')` / `getProviderOption(cached)` / `lib/utils/server/requestContext.getRequestCtx()` 의존 폐기:
  - `getDb({ name: 'MAIN', isUserLess: true }).run(async (client) => client.execute(INSERT ...))` 로 일반 `client.execute` 경로를 그대로 재사용. logger 전용 풀/raw conn 분기 제거
  - **재귀 가드는 `AsyncLocalStorage<boolean>` 로 logger 모듈이 직접 관리** — `logSkipALS.run(true, async () => { await getDb(...).run(...) })` 로 INSERT 자체를 ALS 컨텍스트로 감싸고, `export const isLogSkip = () => logSkipALS.getStore() === true` 를 노출. DB 실행 경로에서 `isLogSkip()` 이 truthy 이면 `insertLogQuery` 호출을 skip 하여 "로그 INSERT → 또 로그" 무한 재귀 차단
  - `LogInfo` 필드 변경 — `startedAt` / `endedAt` 를 ISO 문자열에서 `Date` 객체로 변경, `binds` (직렬화된 바인드 프리뷰) 추가, 사용자 식별 필드(`userId` / `userName` / `role` / `empno`) 를 직접 받도록 변경. `getRequestCtx()` 의존이 사라져 ALS 요청 컨텍스트 없이도 동작. `pagePath` 는 logger 인자에서 제외되어 현재는 `null` 고정
- **호출부** — `lib/auth/auth.ts`, `app/test-0512/_actions/main.ts` 가 `@/lib/db/db-new2` 의 `getDb` 를 사용. v2-split 버전이 신규 호출부의 표준이 됨
- **사이드 변경** — `instrumentation.ts` 의 `registerNode()` 호출은 주석 처리하여 부팅 훅을 일시 비활성화(구버전 `db.ts` 의 `warmupDb` 경로를 끊고 새 흐름에서 부팅 워밍업 필요 여부 재검토). `eslint.config.mjs` 는 신규 분리 파일들이 deep-import 가드와 충돌하지 않도록 정비. `app/test-0512/page.tsx` 는 `myServerAction` 한 개만 호출하는 단순 검증 페이지로 임시 축소

### DB 레이어 재작성 v2 — getDb() 단일 흐름 + globalThis 풀 캐시 (2026-05-21)
PM 리뷰 피드백("`DbDriver` 같은 추상화는 과해, 일단 `getDb()` 한 함수 안에서 위→아래로 읽히게 만들고 필요해지면 나눠라") 을 반영하여 `lib/db/` 를 다시 한 번 단순화. `getDb()` 진입점만 두고, provider 분기를 함수 내부의 `switch` 한 곳으로 모은 두 가지 버전을 신규 작성.

- **`lib/db/db-new.ts` 추가 — `getDb()` 단일 함수 흐름** — `auth → config 파싱 → switch(providerName)` 한 함수 안에서 위에서 아래로 읽힘. provider별 풀 확보 / `DbClient.execute` 정의 / 트랜잭션 commit·rollback / release 까지 같은 블록 안에 둠. `DbDriver` 추상화 / 외부 resolver 폴더 / `createDbAgent` 헬퍼 모두 제거
- **`lib/db/db-new2.ts` 추가 — `runOracle` / `runPg` 분리 버전** — `getDb().run()` 본문은 `auth → config 파싱 → switch` 4~5줄로 압축하고 실제 실행 로직은 `runOracle<R>(...)` / `runPg<R>(...)` 두 함수로 분리. 같은 시그니처 `(name, config, isTransaction, callback, userInfo)` 로 통일하여 일관성 강제. `default` 케이스의 `never` 좁히기로 exhaustive check 유지
- **환경변수 포맷 변경 — `oracledb.createPool` / `pg.Pool` 옵션 키를 그대로 JSON 으로** — 기존 ADO.NET 스타일 `connectionString` 파싱 (`User ID=...;Password=...;Data Source=...`) 폐기. 사내 개발자가 `oracledb` / `pg` 모듈 문서의 옵션명을 그대로 `.env.local` 에 적도록 변경. 예:
  ```
  DB_CONNECTION__MAIN={"providerName":"oracle","connectString":"host:port/svc","user":"...","password":"...","poolMin":10,"poolMax":100,"poolIncrement":1}
  DB_CONNECTION__PG_MAIN={"providerName":"postgres","host":"...","port":5432,"database":"...","user":"...","password":"...","min":10,"max":100,"idleTimeoutMillis":60000,"connectionTimeoutMillis":3000}
  ```
- **`DbConfig` discriminated union** — `OracleConfig` / `PgConfig` 두 타입을 `providerName` literal 로 좁힘. `switch` 의 각 case 안에서 자동으로 좁혀져 별도 캐스팅 없음
- **컬럼 타입 매핑 인라인** — Set/헬퍼 함수 제거. Oracle 은 `metaData.dbTypeName` 문자열 (`'NUMBER' | 'DATE' | 'TIMESTAMP*' | 'VARCHAR2' | 'CHAR' | 'CLOB' | 'BLOB' | ...`) 을 `execute` 의 columns 매핑 안에서 바로 분류 (`NUMBER → number`, `DATE`/`TIMESTAMP*` → `date`, 그 외 → `string`). pg 는 `dataTypeID(oid)` 를 같은 방식으로 인라인 분류 (`16=bool`, `20/21/23/700/701/1700=number`, `1082/1083/1114/1184/1266=date`)
- **풀 캐시 SSOT 를 `globalThis` 로 결정** — 처음에는 모듈 스코프 `Map` 으로 시도했으나, Next.js 가 같은 lib 파일을 `'use server'` Server Action 번들 / NextAuth route handler 번들 등 **별개의 모듈 인스턴스** 로 평가하는 케이스를 실측으로 확인. `auth.ts` 에서 만든 MAIN 풀이 Server Action 호출 시점에는 보이지 않아 `oracledb.createPool({ poolAlias: 'MAIN' })` 가 재실행되며 `NJS-046: pool alias "MAIN" already exists` 발생. 해결: `POOL_CACHE_KEY = '__yms_db_pool_cache__'` (또는 `__myapp_db_pool_cache__`) 키로 `globalThis` 에 `Map<string, oracledb.Pool | PgPool>` 를 매달아 프로세스 전역 SSOT 로 사용. HMR/번들 분리 모두 안전
- **Oracle Thick 모드 초기화는 모듈 스코프 `let` 유지** — `oracledb.initOracleClient()` 가 같은 `libDir` 로 재호출돼도 멱등(no-op) 하게 동작하는 것이 확인되어 globalThis 캐시 불필요. 모듈 인스턴스 수만큼 init 로그가 한 번씩 더 찍히지만 기능상 무해. 사이드이펙트가 거슬리면 `getThickInitFlag()` 헬퍼로 globalThis 로 옮길 수 있음 (TODO 로 남김)
- **운영 로그 추가** — Thick 초기화 시 `Thick Mode OK - ClientVersion: ..., thin: ...`, 풀 생성 시 `oracledb pool created - name: ..., poolMin/poolMax/poolIncrement` 한 줄 로그. rollback / release 실패 시에도 `Transaction Rollback 실패 (name)` / `Connection Release 실패 (name)` 콘솔 에러로 남김
- **호출부 마이그레이션** — `lib/auth/auth.ts`, `app/test-0512/_actions/main.ts` 가 `@/lib/db/db-new` 의 `getDb` 를 사용. `getDb({ isUserLess: true })` 옵션으로 NextAuth `authorize` 콜백처럼 세션이 아직 없는 경로에서 사용자 정보 요구 없이 호출 가능
- **discovered & 문서화** — Next.js dev 모드 + 번들 분리로 인한 모듈 이중 평가가 NJS-046 의 직접 원인임을 실험으로 확인 (모듈 스코프 `let isOracleThickInitialized` 가 두 번 false 로 시작하는 것으로 입증). 운영(`next start`) 에서도 Edge runtime / 워커 프로세스가 있으면 같은 문제가 발생하므로 풀처럼 "두 번 만들면 충돌" 자원은 globalThis SSOT 가 정답

### DB 레이어 단일 파일 재작성 + Postgres 추가 (2026-05-19)
PM 피드백("factory/provider 추상화는 과해, 사내 개발자가 한 파일에서 바로 읽고 고칠 수 있는 구조로") 을 반영하여 `lib/db/` 를 단일 진입 파일 위주로 재구성. provider 추상화 레이어 / 외부 resolver 폴더 / 클래스 기반 logger 를 모두 걷어내고, oracle/postgres 분기를 한 곳에 모은 `db.ts` 하나로 통일.

- **`lib/db/db.ts` 신규 단일 진입점** — `getDb({ name }).run(cb)` / `.runTx(cb)` 콜백 패턴. 콜백 인자로 `DbAgent { fill, execute }` 와 `RequestCtx` (= userInfo) 가 함께 주입되어 호출부에서 두 번째 인자만 받아 쓰면 됨. `transaction(async () => ...)` ALS 모델 폐기 — 콜백 안에서만 동일 커넥션을 공유하므로 cross-call enlist / 중첩 차단 코드가 통째로 사라짐
- **공통 `QueryResult` 1종으로 통일** — fill / execute 가 동일하게 `{ columns, rows, affectedCount }` 를 반환. fill 은 `columns/rows` 채움 + `affectedCount=0`, execute 는 빈 배열 + `affectedCount` 만 채움. 호출부 분기 없이 같은 분해 패턴 사용 가능
- **provider 분기를 `getProviderOption(cached)` 한 군데로 집중** — oracle/postgres 의 `getConnection / release / begin / commit / rollback / rawExecute` 를 클로저 6개로 묶어 반환. `run` / `runTx` 는 provider 무관한 평탄한 lifecycle (acquire → begin? → cb → commit?/rollback? → release) 코드가 됨. Conn 타입은 `unknown` 유지 — 같은 ops 가 만든 conn 이 같은 ops 로 되돌아오는 닫힌 흐름이라 외부 캐스팅 불필요
- **`lib/db/envResolver.ts` 신규, `lib/db/resolvers/env.ts` 의 역할을 흡수** — `DB_CONNECTION__<NAME>` JSON + ADO.NET 스타일 `connectionString` 파싱. `providerName: 'oracle' | 'postgres'` 만 지원. postgres 의 `Data Source` 는 `host:port/database` 형식으로 그대로 사용
- **postgres provider 인라인 추가** — `pg` 모듈 직접 사용. `:name` 바인드를 `$1, $2` 로 변환하는 `toPgQuery` 헬퍼로 SQL 작성은 oracle/postgres 동일 표기. 컬럼 타입은 `dataTypeID(oid)` → `'string' | 'number' | 'date'` 매핑
- **Oracle Thick 모드 초기화 & HMR-safe 풀 캐시** — `ensureOracleThick()` (`globalThis.__myapp_oracle_thick_initialized__`) + `getPoolCache()` (`globalThis.__myapp_db_pool_cache__`). Next.js dev HMR 로 모듈이 재평가되어도 풀/init 중복 발생 안 함
- **`lib/db/logger-new.ts` 함수 1개로 축소** — `insertLogQuery(fields)` 만 export. `db.ts` 의 `getPool('MAIN')` + `getProviderOption(cached)` 를 직접 호출하여 **같은 풀에서 자기 커넥션을 빌리고** `autoCommit=true` 로 INSERT. 사용자 트랜잭션과 enlist 되지 않아 rollback 영향을 받지 않음. agent 의 fill/execute 를 거치지 않고 `providerOp.rawExecute` 를 직접 호출하므로 무한 재귀 (로그 INSERT → 또 로그) 차단. 모듈 스코프 캐시/Promise 상태도 제거 — `getPool` 내부의 Map 캐시가 이미 hot path 비용을 흡수
- **에이전트 메서드 인라인** — `createDbAgent` 의 fill / execute 가 각자 try/catch + `insertLogQuery` 호출 블록을 직접 들고 있도록 펼침. 헬퍼 `dbAgentRun` 제거. 한 메서드를 위에서 아래로 읽으면 "exec → 성공 로그 + return / 실패 로그 + throw" 흐름이 한 화면에서 끝남
- **신규 export** — `getPool`, `getProviderOption`, `CachedPool` (logger-new.ts 가 풀/ops 를 재사용하기 위해 필요)
- **deprecated 파일 (다음 PR 에서 제거 예정)** — `lib/db/factory.ts`, `lib/db/providers/oracle.ts`, `lib/db/providers/postgres.ts`, `lib/db/providers/index.ts`, `lib/db/resolvers/env.ts`, `lib/db/logger.ts`, `lib/db/errors.ts`, `lib/db/types.ts`. 호출부 마이그레이션 완료 후 정리
...
> ⚠️ **초기 설계 기록.** 일부 내용은 상단 **"appId/deptSite 라우팅·인증 DB 연동 전환 + 5자리 넘버링 (2026-06-03)"** 섹션으로 대체되었다. 현재 기준: appId는 5자리(`A00001`), `getDeptSitesCached`(60초 캐시) → `getDeptSites()`(실시간 DB 조회), `token.role`/`appIds` 기반 인가 → 세션 `hasKr/Jp/CnFlag` 기반, default deptSite redirect → `'KR'` 기본값. 아래는 도입 당시의 원본 설계로 보존한다.
