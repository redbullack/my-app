    ...
    return {
        async run<R>(callback: (client: DbClient, userInfo?: Session) => Promise<R>): Promise<R> {
            ...
            switch (config.providerName) {
                case 'oracle':   return runOracle(name, config, isTransaction, callback, userInfo)
                case 'postgres': return runPg(name, config, isTransaction, callback, userInfo)
                default: {
