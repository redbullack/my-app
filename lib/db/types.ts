 ...
 * 호출자(서버 액션·라우트 핸들러) 에게는 노출되지 않는다 — IDbClient.query/execute 는
 * 옵션 인자 자체를 받지 않으며, factory 가 lifecycle 안에서 traceId 와 tx conn 을 자동 주입한다.
 *
 * maxRows/timeoutMs 같은 쿼리 단위 옵션은 의도적으로 제거했다 — 행 수 제한은 SQL 의
 * `LIMIT`/`ROWNUM` 으로, 타임아웃은 DB/세션 설정으로 다루는 것이 운영상 더 명확하다.
 */
export interface QueryOptions {
  /** 로그 상관관계용 trace ID. (현재 미사용 — 향후 분산 추적 도입 시 활용) */
  traceId?: string
