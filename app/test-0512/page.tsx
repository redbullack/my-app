...
export default function Test0512Page() {
  const {executeAction, data, error, errorKind, isError, isLoading, reset} = useAction()

  // const handleMyServerAction = useCallback(async () => {
  //   const result = await myServerAction('This is from page.tsx ~')
  //   result.columns.forEach((col) => {
  //     console.log(`col.name: ${col.name}, col.type: ${col.type}`)
  //   })
  //   result.rows.forEach((row) => {
  //     console.log(`COL1: ${row.COL1}, COL2: ${row.COL2}, COL3: ${row.COL3}, USER_ID: ${row.USER_ID}, COL1: ${row.BIND_COL}`)
  //   })
  ...
  const handleMyServerAction = useCallback(async () => {
    const result = await myServerAction(`This is from page.tsx ~`)
    result.rows.forEach((row) => {
        console.log(`COL1: ${row.COL1}, COL2: ${row.COL2}, COL3: ${row.COL3}, USER_ID: ${row.USER_ID}, BIND_COL: ${row.BIND_COL},`)
    })
    alert(`result: ${result.rows.length} !`)
  }, [])

//   const handleMyTxServerAction = useCallback(async () => {
//     const result = await myServerAction('This is from page.tsx ~')
//     alert(`result: ${result.rows.length} !`)
//   }, [])
